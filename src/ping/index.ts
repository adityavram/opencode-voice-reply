// Phone ping orchestrator — coordinates three ping modes:
//   textPing(): Telegram text → poll for reply → inject into session
...
// All modes inject the user's response back into the opencode session.
...
// Backup cleanup — polls Twilio call status in case the Gather webhook never fires.
// Also has a hard max wait of 120s to prevent zombie servers.
