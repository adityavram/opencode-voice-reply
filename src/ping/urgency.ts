// Urgency classifier — determines whether a permission prompt is "high" or "low"
// urgency. Used by the escalation ping mode to decide whether to escalate from
// a Telegram text to a phone call.
// Heuristic first (regex patterns for destructive/irreversible ops); if
// ambiguous, falls back to LLM classification. Intentionally conservative —
// better to over-classify as "high" than miss a destructive operation.
...
// Defaults to "low" on any error (we already sent the text, so the user is aware).
