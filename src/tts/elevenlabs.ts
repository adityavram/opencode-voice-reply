...
// Exports speak(text) for local playback and synthesize(text) for raw audio
// bytes (used by the phone ping for call audio generation).
...
// Synthesize text to MP3 audio bytes. Used by the phone ping to generate
// audio for Twilio calls without playing locally.
...
// Synthesize text and play it locally. Audio is written to a temp file,
// played via the configured player (default: afplay), then cleaned up.
