// Plugin entry point — ties together voice reply, permission notification,
// speech interruption, and the /voice and /ping slash commands.
// All errors are caught so a TTS or ping failure never breaks the session.
...
// On macOS, both 'rename' and 'change' events fire inconsistently from fs.watch,
// so we trigger on any event and compare the file list to detect new recordings.
...
// Handles all three permission event types (permission.updated, permission.asked,
// permission.v2.asked) and reads from both `properties` (v1) and `data` (v2)
// fields to be resilient to schema variations.
